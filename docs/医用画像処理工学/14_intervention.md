...
この問題は、次の2つの問いに分解できます。この2つを区別することが、各手法を理解する鍵になります。

---
 
#### 2.1.1. 問1: 光線をどう飛ばすか - 投影法
...
#### 2.1.2. 問2　光線上のボクセルをどう「合成」するか - レイキャスティング法
...
この「合成のしかた」を変えるだけで、まったく異なる画像が得られます。
上図③に注目してください。光線の飛ばし方は同一なのに、拾った値を「最大値で1つ選ぶ」か「不透明度で全部積分する」かの違いだけで、まったく異なる画像が生まれています。
...
#### 2.2.1. サーフェスレンダリング (Surface Rendering)
...
#### 2.2.2. ボリュームレンダリング (Volume Rendering)
