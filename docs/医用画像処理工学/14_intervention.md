...
この問題は、次の**2つの問い**に分解できます。この2つを区別することが、各手法を理解する鍵になります。

<div style="border: 1px solid #ccc; padding: 10px; margin-bottom: 15px;">
  <font color="red"><strong>3D可視化を決める2つの問い</strong>:</font><br>
  <ul style="margin-top: 5px; margin-bottom: 0; padding-left: 20px;">
    <li style="color: black;"><strong>問1: 光線をどう飛ばすか？ → 投影法（平行投影 / 透視投影）</strong></li>
    <li style="color: black;"><strong>問2: 光線上に並んだボクセルを、どう1つの色にまとめるか？ → 合成方法（MIP / MinIP / サーフェス / ボリューム）</strong></li>
  </ul>
</div>

MIPもサーフェスレンダリングもボリュームレンダリングも、**「光線を飛ばす」ところまでは全く同じ**です。
違うのは **問2、すなわち光線に沿って集めた値をどう合成するか**、その一点だけです。
...
ここからが本題です。1本の光線（＝画面の1画素）は、体内を貫きながら多数のボクセル値を拾ってきます。
下図のように、脳MRIを貫く光線は「頭蓋骨 → 脳実質 → 腫瘍 → 脳実質 → 頭蓋骨」という値の列を得ます。

**この値の列を、たった1つの色（画素値）にまとめる規則** を **合成方法（compositing）** と呼び、
その規則の違いが、そのまま可視化手法の違いになります。

<figure>
  <img src="../img/14-06.png" alt="合成方法の比較" width="900" height="auto">
  <figcaption>同じ1本の光線に対する4つの合成方法。左端が光線上の信号プロファイル。MIP・MinIP・サーフェスは「1点を選ぶ」、ボリュームレンダリングだけが「全点を積分する」。</figcaption>
</figure>

合成方法は、大きく **選択型** と **積分型** の2種類に分けられます。

<div style="border: 1px solid #ccc; padding: 10px; margin-bottom: 15px;">
  <font color="red"><strong>最重要ポイント</strong>:</font><br>
  <ul style="margin-top: 5px; margin-bottom: 0; padding-left: 20px;">
    <li style="color: black;"><strong>選択型（MIP・MinIP・サーフェスレンダリング）: 光線上から<u>ボクセルを1つだけ選ぶ</u> → 選ばれなかったボクセルの情報は捨てられる = <u>内部構造は見えない</u></strong></li>
    <li style="color: black;"><strong>積分型（ボリュームレンダリング）: 光線上の<u>すべてのボクセルを不透明度で重み付けして足し合わせる</u> → <u>内部構造が透けて見える</u></strong></li>
  </ul>
</div>

| 手法 | 合成方法 | 型 | 内部構造 | 主な用途 |
|---|---|---|---|---|
| **MIP** (最大値投影法) | 光線上で**最も高い**値を採用 | 選択型 | 見えない | 造影血管、石灰化、結石 |
| **MinIP** (最小値投影法) | 光線上で**最も低い**値を採用 | 選択型 | 見えない | 気管・気管支、胆管 |
| **サーフェスレンダリング** | **最初に閾値を超えた**点を採用し、陰影をつける | 選択型 | 見えない | 骨、臓器の形状、3Dプリンタ |
| **ボリュームレンダリング** | **すべて**の点を不透明度で重み付けして合成 | **積分型** | **見える** | 血管と臓器の同時表示、術前計画 |

> **なぜサーフェスレンダリングでは内部が見えないのか？**
> 上図の「Surface (iso)」を見てください。光線は**最初に閾値を超えた頭蓋骨**で止まってしまい、
> その奥にある**腫瘍には到達していません**。
> 「表面しか描画しないため内部の構造がわからない」という短所は、
> **合成方法が「1点で打ち切る」ものである以上、必然的にそうなる**のです。
> 一方ボリュームレンダリングは、頭蓋骨を半透明にしておけば、光線はその奥の腫瘍まで届きます。

> **MIPとサーフェスレンダリングは実は親戚である**
> どちらも「光線上から1点を選ぶ」という同じ枠組みです。
> 選ぶ規則が「最大値」か「最初に閾値を超えた点」かの違いにすぎません。
> このため、レイキャスティング法のプログラムは、**合成部分の数行を書き換えるだけで
> MIPにもMinIPにもサーフェス表示にもなります**（演習で実際に確認します）。
...
対象物の**「表面」だけを抽出**して描画する手法です。閾値を境界として、対象物の内と外を切り分けます。

サーフェスレンダリングには、目的地は同じでも**2つの実装ルート**があります。

**ルートA：ポリゴンモデルに変換してから描く（Marching Cubes法）**
...
*   **代表的なアルゴリズム:** **`Marching Cubes法`**
    ...
    3.  その組み合わせパターン（2^8 = 256通り）に応じて、あらかじめ定義されたポリゴン（三角形のメッシュ）を立方体内に配置（Marching）します。
    4.  これを全ボクセル領域で繰り返すことで、対象物全体の表面を表現するポリゴンモデルが生成されます。
*   ボクセルデータを**一度ポリゴン（幾何モデル）に変換**するのが特徴です。一度変換してしまえば、あとはGPUが得意とする三角形の描画になるため高速で、**3DプリンタのSTLデータもこの形式**です。

**ルートB：光線を飛ばし、閾値を超えた点で止める（レイキャスティングによる等値面表示）**

*   ポリゴンを作らず、光線が**最初に閾値を超えた位置**をそのまま表面とみなして陰影をつけます。
*   ボリュームレンダリングと同じプログラムの、**合成部分を「1点で打ち切る」に変えただけ**で実現できます。**（演習ではこちらを実装します）**
...
ボリュームレンダリングでは各ボクセルに**透明度**というパラメータを設定することで、**物体の内部も表示可能にします**。
これが、前述の「積分型」であることの直接の帰結です。

*   **代表的なアルゴリズム:** **`レイキャスティング法 (Ray-Casting)`**
    1.  画面（スクリーン）の各画素から、視点とは反対方向に仮想的な光線（Ray）を飛ばします。
    2.  この光線がボクセルデータを通過する際に、各ボクセルの値（CT値など）と**伝達関数（Transfer Function）**に基づいて、色（Color）と不透明度（Opacity）を計算します。
    3.  光線に沿って色と不透明度を**蓄積・合成**していき、最終的な画素の色を決定します。

**伝達関数 (Transfer Function) とは？**
    ボクセルの値（入力）を、色と不透明度（出力）に変換するための関数。どのCT値の組織を、どの色で、どのくらい透明/不透明に表示するかを定義します。この設定次第で、骨だけを強調したり、血管と臓器を同時に表示したりと、多彩な表現が可能になります。

> **サーフェスレンダリングの「閾値」は、伝達関数の最も単純な形とみなせます。**
> 「閾値より下は完全に透明 (不透明度0)、上は完全に不透明 (不透明度1)」という
> **階段状の伝達関数**を設定したものが、サーフェスレンダリングにほかなりません。
> 伝達関数をなだらかな曲線にすることで、初めて「半透明」という表現が生まれ、内部が見えるようになります。
...
| | 光線の飛ばし方 | 光線上の合成 | 内部が見えるか | データ形式 |
|---|---|---|---|---|
| **MIP / MinIP** | 平行 or 透視 | 最大値 / 最小値を**選ぶ** | × | ボクセルのまま |
| **サーフェスレンダリング** | 平行 or 透視 | 閾値を超えた最初の点を**選ぶ** | × | ポリゴンに変換 (Marching Cubes) <br> またはボクセルのまま (レイキャスト) |
| **ボリュームレンダリング** | 平行 or 透視 | 全点を不透明度で**積分する** | **○** | ボクセルのまま |

**「光線を飛ばし、拾った値をどう合成するか」** — この一点さえ押さえれば、
すべての可視化手法は同じ枠組みの上に整理できます。
次の演習では、実際にレイキャスティング法を実装し、
**合成部分の数行を書き換えるだけで表示が劇的に変わること**を体験します。