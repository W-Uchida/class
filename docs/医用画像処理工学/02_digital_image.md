...
<figure>
  <img src="../img/02-02.png" alt="2進数カウント" width="500" height="auto">
...
元の信号は正弦波で、周波数は$f_{max}=1 Hz$とします。この時、ナイキスト周波数$f_n=2*f_{max}=2 Hz$です。

サンプリング間隔0.33秒（$f_s=3.0Hz$）：ナイキスト定理を満たしており、赤いサンプル点から元の信号（青線）が正確に復元できています。

サンプリング間隔0.5秒（$f_s=2.0Hz$）：ナイキスト定理の境界条件（最高周波数の2倍）ですが、まだ元の信号を正確に復元できています。

サンプリング間隔0.67秒（$f_s=1.5Hz$）：ナイキスト定理を満たしていないため、エイリアシングが発生し、元の信号とは異なる低周波の信号（青線）が復元されています。

サンプリング間隔0.8秒（$f_s=1.25Hz$）：さらにサンプリング周波数が低くなり、より顕著なエイリアシングが発生しています。
...
### **量子化ノイズと信号対雑音比（SNR・PSNR）：**
アナログ信号をデジタル化する際の「量子化誤差」はノイズとみなすことができ、画像や信号の品質に影響を与えます。この品質を評価する指標として、主に**信号対雑音比（SNR）**や**ピーク信号対雑音比（PSNR）**が用いられます。

ビット深度（$B$ ビット）が大きくなるほど、これらの値は向上（ノイズが減少）します。評価の基準によって、以下の2つの指標が使い分けられます。

#### **1. 正弦波に対する SNR（Signal-to-Noise Ratio）**
音声や通信などの一般的な信号処理分野でよく用いられます。入力信号がフルスケール（最大振幅）の**正弦波**であると仮定した場合、理想的なA/D変換における理論的なSNRは次式で表されます。

$$
\text{SNR} \approx 6.02B + 1.76 \quad \text{[dB]}
$$

これは「ビット深度が1ビット増えるごとに、SNRが約6dB改善する」という有名な法則です。

#### **2. 画像評価に用いられる PSNR（Peak Signal-to-Noise Ratio）**
画像処理の分野では、信号の平均的な強さ（実効値）ではなく、画素が取り得る**最大値（ピーク値）**を基準としたPSNRがしばしば用いられます。理想的な $B$ ビット量子化のみによる理論的なPSNRは、次式で近似できます。

$$
\text{PSNR} \approx 6.02B + 10.8 \quad \text{[dB]}
$$

<figure>
  <img src="../img/02-12.png" alt="PSNR" width="700" height="auto">
...
<figure>
  <img src="../img/02-13.png" alt="第2回演習" width="300" height="auto">
