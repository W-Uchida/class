...
超音波は、その高い周波数特性により、指向性を高く保ちやすく、エネルギーを特定の方向に集中させやすいという利点があります。また、音波の強度$I [W/m^2]$（単位面積あたりの音響パワー）は、媒質（空気の分子など）が振動する変位振幅$A [m]$、その周波数$f [Hz]$、媒質の音響インピーダンス$Z [kg/m^2s]$と以下の関係があります。
...
-   **表示方法**：特定の1本の超音波ビームライン上で受信された**反射エコーの強さ（振幅: Amplitude）を縦軸に、探触子からの深さ（時間に比例）を横軸**にとって、波形として表示するモードです。
-   **特徴**：
...
-   **表示方法**：Aモードで得られる反射エコーの振幅（強さ）を、輝点（点）の明るさ（輝度: Brightness）に変換し、その輝点をエコーが返ってきた深さの位置に表示します。この超音波ビームを電子的に、あるいは機械的に走査（スキャン）させることで、多数のビームラインからの情報を組み合わせて**二次元の断層画像として表示**するモードです。
-   **特徴**：
    -   **二次元の断層像**を提供するため、体内の臓器の形態や構造を視覚的に把握することができます。
    -   現在の超音波診断装置における**最も基本的な表示モードであり、最も頻用される表示法**です。
    -   探触子を動かすことで、**リアルタイムに体内の様子を観察することが可能**です。これにより、臓器の動きや血流の状態（カラードプラと併用時）なども評価できます。

#### 4.1.3 Mモード（Motion Mode）

-   **表示方法**：特定の1本の超音波ビームライン上で、時間経過に伴う反射体の動きを記録・表示するモードです。Bモード画像上で関心のある音線を設定すると、その**音線上のエコー輝度を縦軸（深さ方向）にとり、横軸を時間軸**として、輝度情報を連続的にスクロール表示します。
-   **特徴**：
...
| 項目 | Aモード | Bモード | Mモード |
|------|---------|---------|---------|
| 名称の由来 | Amplitude（振幅） | Brightness（輝度） | Motion（動き） |
| 使用する波 | **パルス波** | **パルス波** | **パルス波** |
| 音線数 | 単一音線 | 複数音線（走査） | 単一音線 |
| 表示の次元 | 一次元（深さ） | **二次元**（断層像） | 一次元（深さ）＋時間軸 |
| 横軸 | 深さ（時間） | 空間的幅（走査方向） | 時間 |
| 縦軸 | エコー振幅 | エコー輝度（輝点） | 深さ（輝度） |
| 断層像の取得 | **不可** | **可能** | 不可 |
| 時間分解能 | 中 | 低（複数音線処理のため） | **最高** |
| 主な用途 | 距離・振幅の精密測定 | 臓器形態の観察 | 心臓弁・心室壁の運動計測 |
| 代表的な臨床応用 | 眼軸長測定など | 腹部・産科・心臓エコー全般 | 弁膜症評価・左室駆出率算出 |
...
$$
f_1 = f_0(\frac{c+v\cos\theta}{c})
$$

**Step 2: 受信時** 
移動する血球→探触子：探触子が受け取る周波数は
$$
f_2 = f_1(\frac{c+v\cos\theta}{c})
$$
