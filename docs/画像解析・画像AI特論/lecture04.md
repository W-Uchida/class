...
3. **フォトンカウンティングCT**（PCCT: Photon Counting CT）
      - 検出器: フォトンカウンティング（光子計数型）検出器
      - 特徴（検出器）:
        - 従来のエネルギー積分型検出器と異なり、X線光子一つ一つのエネルギーと数を直接計測する。
        - 検出器は通常、カドミウムテルル（CdTe）やシリコン（Si）などの半導体材料を使用する。
        - 画素サイズを非常に小さくできるため、超高空間分解能を実現。
      - スライス取得: マルチスライスCTと同様に複数スライスを同時に取得可能（多くはヘリカルスキャンに対応）
      - 撮影方式: 主にヘリカル方式
      - 特徴（検出器以外）:
        - 高空間分解能: 微細構造（骨小柱、血管壁のプラーク、肺の末梢病変など）の描出能力が格段に向上する。
        - 低ノイズ・高コントラスト: 検出効率が高く、電子ノイズが少ないため、低線量でも高画質の画像が得られる。X線光- 子エネルギーを弁別できるため、物質によるX線吸収特性の違いをより明確に捉えることができ、コントラストが向上- する。
        - スペクトル情報（多エネルギー情報）の活用: X線光子のエネルギーを直接計測するため、異なるエネルギー帯域の画- 像を再構成できる。これにより、以下のアプリケーションが可能になる。
        - 物質弁別: 骨、軟部組織、造影剤などの物質ごとにX線吸収特性が異なることを利用し、これらを分離・定量化できる- （例: 骨除去画像、ヨードマップ、尿酸結晶の検出）。
        - 単色X線画像（Monochromatic Image）: 特定のエネルギーのX線で撮影したかのような画像を再構成できるため、- ビームハードニングアーチファクトの低減や造影剤効果の最適化が可能。
        - 定量性: 組織の組成や造影剤濃度などをより正確に定量評価できる。
        - 低被ばく: 検出効率の向上とノイズの低減により、従来のCTよりも低いX線線量で同等以上の画質が得られる可能性- がある。
...
### 2.1 CT値（Hounsfield Unit: HU）の基本概念
